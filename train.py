...
def train(train_loader,model,criterion,optimizer,epoch,valid_accuracy,start):
    ...
    for i,(images,target,fnames) in enumerate(train_loader):
        ...
        message = (
                    f"train {i:5.1f} {epoch:6.1f}        |"
                    f"      {losses.avg:.3f}     |"
                    f"      {valid_accuracy[0]:.3f}     |"
...
if __name__ == '__main__':
    ...
    model = timm.create_model(config.model_name, pretrained=True,num_classes=config.n_classes)
    ...
    #scheduler =lr_scheduler.StepLR(optimizer, step_size= 3, gamma=0.00085)
    ...
    for epoch in range(0,config.epochs):
        ...
        Accuracy_list.append(val_metrics[0].tolist())
        scheduler.step() 
        
    #Dictionary
    dict_loss = {'Train_Accuracy':Accuracy_list,'Train Loss':loss_list}
    with open(f"logs\\Learned files\\{config.store_json}", "w", encoding="utf-8") as f:
        json.dump(dict_loss, f, ensure_ascii=False)


    ## Saving the model
    filename = f"logs\\Learned files\\{config.checkpoints}"
